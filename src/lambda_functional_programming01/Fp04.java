...
public class Fp04 {
    public static void main(String[] args) {
        ...
        System.out.println(verilenikiSayıArasındakiCiftSayılarınToplamını(11, 5));
        System.out.println(ikiSayiArasindakiTumSayilarinRakamlarinToplami(13,11));
   ...
   //1.yol
    ...
    public static int get7den100eToplam02() {
        return IntStream.range(7, 100).reduce(Math::addExact).getAsInt();
    }
...
// (5 factorial = 1*2*3*4*5  ==> 5! = 1*2*3*4*5)
    public static int faktoryelHesapla(int x) {
        ...
        System.out.println("0`den buyuk deger giriniz");

        return 0;
    }

    //4) Verilen iki sayı arasındaki çift sayıların toplamını bulan bir method oluşturun.
    public static int verilenikiSayıArasındakiCiftSayılarınToplamını(int x, int y) {
     int z=0;
       if (x > y) {
           z=x;
           x=y;
